...
function [row_mean,row_var,col_mean,col_var] = histogram_analyse (data)
    ...
    for row = 1:rows
        ...
        %debugging stuff
        actual_num = [actual_num; this_row];
        %
        ...
        %row_var(row)=sum(this_row.^2)/intervals^2 - row_mean(row)^2;
        row_var(row)=sum( (this_row -row_mean(row)).^2 ) / intervals;
    end
